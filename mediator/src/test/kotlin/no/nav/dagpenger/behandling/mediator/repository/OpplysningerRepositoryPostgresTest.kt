...
import no.nav.dagpenger.behandling.TestOpplysningstyper.testRegelverk
...
class OpplysningerRepositoryPostgresTest {
    ...
    fun `skal slette fjernede opplysninger som er utledet av i flere nivåer`() {
        withMigratedDb {
            ...
            val regelsett1 =
                vilkår(testRegelverk, "vilkår en") {
                    regel(a) { innhentes }
                    regel(d) { innhentes }
                    regel(b) { enAv(a) }
                    regel(c) { enAv(b, d) }
            ...
            val regelsett2 =
                vilkår(testRegelverk, "vilkår to") {
                    regel(e) { enAv(c) }
                    regel(f) { alle(e, b, a) }
                }
            val opplysninger = Opplysninger()
